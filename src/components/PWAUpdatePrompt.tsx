...
export const PWAUpdatePrompt = () => {
  ...
  return (
    <div
      className="fixed inset-x-0 bottom-[calc(5.75rem+env(safe-area-inset-bottom))] z-50 px-4 sm:bottom-[calc(1.5rem+env(safe-area-inset-bottom))]"
    ...
    >
      <div className="mx-auto flex w-full max-w-md items-center gap-3 rounded-2xl border border-border bg-background/95 p-3 shadow-[0_16px_40px_-22px_rgba(0,0,0,0.75)] backdrop-blur supports-[backdrop-filter]:bg-background/85">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs leading-snug text-muted-foreground">Restart Poscal to switch to the latest version.</p>
        </div>
        <Button size="sm" onClick={updateApp} disabled={isUpdating} className="shrink-0">
          {isUpdating ? "Restarting" : "Restart"}
        </Button>
      </div>
    </div>
  );
